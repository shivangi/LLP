...
var zoomInfo = {"district":7,"preschooldistrict":7,"block":8,"cluster":9,"school":11,"project":9,"circle":11,"preschool":13};
...
var center={"lat":34.07,"lon":77.58};
var defaultid="7000";
var childInfo=[];
      ...
      var callback = {
        on: { success: 
          function(id, o) {
            ...
            document.getElementById("systext").innerHTML='<p><b>Number of Stories Shared :</b>'+sysinfo["numstories"]+' , <b>Number of Photos Shared :</b>'+sysinfo["numimages"]+'</p>';
          },
...
function plotPoints(pointInfo)
{
  for( type in pointInfo)
  {
    ...
    if( type=='village')
      populateVillage(pointInfo[type]);
  }
}
...
function populateVillage(info)
{
  var select = document.getElementById("village");
  var count=1;
  for( var d in info){
    select.options[count] = new Option(info[d].name);
    count = count+1;
  }
}
...
function changeFocus(parentType,childType)
{
  ...
  if (parentType == "village")
    getVillageBoundary(id,parentType,childType)
  var marker;
  if(parentType =="village")
  {
...
function getVillageBoundary(parentId,parentType,childType)
{
  YUI({base: 'yui/build/',
    timeout: 50000}).use("io-base","json-parse",
    function(Y, result) {
